...
logger = logging.getLogger('sentence_demo_augmentation')
logger.setLevel(logging.DEBUG)

def check_all_agree(raw_annotations, query="Q2: Safe or Unsafe"):
    annotations = raw_annotations[query]
    return len(set([a['sentence_label'] for a in annotations])) == 1
...
def main(args):
    ...
    # select 'ambiguous' sample in demonstrations of each sentence class
    for class_id, demos in enumerate([
        [d for d in output_demo if d['sentence_label'] == "unsafe"],
        [d for d in output_demo if d['sentence_label'] == "safe"],
    ]):
        ...
        # sort and index
        _demos = sorted([(d,v) for d,v in zip(demos, vars)], key=lambda d: d[1], reverse=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_demo_fname",
                        type=str,
                        default="kosbi_sentence_iter1.json")
